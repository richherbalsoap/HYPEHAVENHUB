...
def razorpay_direct_checkout(request):
    ...
    # Check if address_id was explicitly passed from frontend
    address_id = data.get('address_id')
    address = None
    if address_id:
...
def verify_payment(request):
    ...
    try:
        ...
        if not local_order_id and razorpay_order_id:
            found_order = (
                Order.objects.filter(payment__gateway_response__icontains=razorpay_order_id).first() or
                Order.objects.filter(notes__icontains=razorpay_order_id).first()
            )
            if found_order:
                local_order_id = found_order.order_id
        ...
        if not all([razorpay_payment_id, razorpay_order_id, razorpay_signature, local_order_id]):
            ...
            return redirect('/checkout/?error=missing_credentials')
        ...
        with transaction.atomic():
            ...
            if request.user.is_authenticated and order.user != request.user and order.user.username != 'guest_checkout':
                ...
                return redirect('/checkout/?error=unauthorized')
            ...
            try:
                ...
                return redirect('/checkout/?error=signature_mismatch')
...
def customize_verify_payment(request):
    ...
    try:
        ...
        if not all([razorpay_payment_id, razorpay_order_id, razorpay_signature, local_order_id]):
            if is_ajax:
                return JsonResponse({'success': False, 'message': 'Missing payment credentials.'})
            return redirect('/customize/?error=missing_credentials')
        ...
        with transaction.atomic():
            ...
            try:
                ...
                return redirect('/customize/?error=signature_mismatch')
        ...
        return redirect('/customize/?error=order_not_found')
    except Exception as e:
        ...
        return redirect('/customize/?error=verification_error')
